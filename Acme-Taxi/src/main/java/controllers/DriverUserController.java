...
public class DriverUserController extends AbstractController {
	...
	public ModelAndView list(final HttpServletRequest request) {
		...
		final Integer total = this.driverService.countDriversReviewable();
		drivers = this.driverService.findDriversReviewable(pageable);
		res = new ModelAndView("driver/list");
		res.addObject("drivers", drivers);
		res.addObject("total", total);
		res.addObject("requestURI", "driver/user/list.do");
		return res;
	}
}
