...
public class ReviewDriverController extends AbstractController {
	...
	public ModelAndView list(final HttpServletRequest request) {
		...
		final Integer total = this.reviewService.countReviewsByDriverId(this.driverService.findByPrincipal().getId());
		reviews = this.reviewService.findReviewsByDriverId(this.driverService.findByPrincipal().getId(), pageable);
