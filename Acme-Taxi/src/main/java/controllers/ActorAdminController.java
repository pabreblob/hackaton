...
public class ActorAdminController extends AbstractController {
	...
	public ModelAndView listBanned(final HttpServletRequest request) {
		Pageable pageable;
		Direction dir = null;
		Integer pageNum = 0;
		final String pageNumStr = request.getParameter(new ParamEncoder("row").encodeParameterName(TableTagParameters.PARAMETER_PAGE));
		...
		final ModelAndView res = new ModelAndView("actor/listBanned");
		res.addObject("actors", this.actorService.getBannedActor(pageable));
		res.addObject("total", this.actorService.countBannedActor());
		return res;
	}
