...
<display:table name="announcements" id="row" requestURI="${requestURI}" pagesize="5" class="displaytag" sort="external" partialList="true" size="${total}">
...
<display:column property="origin" sortable="true" sortName="origin" title="${originHeader}"/>
<spring:message code="announcement.destination" var="destinationHeader"/> 
<display:column property="destination" sortable="true" sortName="destination" title="${destinationHeader}"/>
<spring:message code="announcement.price" var="priceHeader"/> 
<display:column sortable="true" sortName="pricePerPerson" title="${priceHeader}">
...
<display:column>
<jstl:if test="${empty row.attendants and row.moment > currentDate and !row.cancelled}">
	<a href="announcement/user/edit.do?announcementId=${row.id}"><spring:message code="announcement.edit"/></a>
</jstl:if>
</display:column>
<display:column>
<jstl:if test="${!row.cancelled and row.moment > currentDate}">
	<a href="announcement/user/cancel.do?announcementId=${row.id}"><spring:message code="announcement.cancel"/></a>
</jstl:if>
</display:column>
</jstl:if>
...
<jstl:if test="${row.moment > currentDate and !row.cancelled}">
	<a href="announcement/user/dropout.do?announcementId=${row.id}"><spring:message code="announcement.dropout"/></a>
</jstl:if>
</display:column>
</jstl:if>
